...
namespace EncryptedMessaging
{
    public class Messaging
    {
        ...
        private void ExecuteCommand(ulong chatId, Message message)
        {
            ...
            else if (message.Type == MessageType.FirebaseTokenChange)
            {
                message.Contact.FirebaseToken = Encoding.Unicode.GetString(message.Data);
            }
            else if (message.Type == MessageType.DeviceTokenChange)
            {
                message.Contact.DeviceToken = Encoding.Unicode.GetString(message.Data);
            }
        ...
        public void SendStartVideoGroupCall(byte[] call, Contact toContact) => SendMessage(MessageType.StartVideoGroupCall, call, toContact);
        ...
        public void NotifyFirebaseTokenChange(Contact toContact, string firebaseToken)
        {
            var data = firebaseToken.GetBytes();
            SendMessage(MessageType.FirebaseTokenChange, data, toContact);
        }

        public void NotifyDeviceTokenChange(Contact toContact, string deviceToken)
        {
            var data = deviceToken.GetBytes();
            SendMessage(MessageType.DeviceTokenChange, data, toContact);
        }
#if DEBUG_A
