...
namespace EncryptedMessaging
{
    ...
    public class Message
    {
        ...
        {
            get => Context.SecureStorage.ObjectStorage.LoadObject(typeof(string), "t" + PostId) as string;
    ...
    public class MessageFormat
    {
        ...
        public enum MessageType : byte
        {
            ...
            ReplyToMessage,
            FirebaseTokenChange,// This info is sent to the contact to inform me that my firebase token has been updated
            DeviceTokenChange// This info is sent to the contact to inform me that my device token has been updated
        }
            ...
            if (version <= 1)
            {
                ...
                try
                {
                    ...
                    if (authorIndex > participants.Count - 1)
                    {
                        ...
                        message = new Message(_context, contact, type, author, dateAndTime, data, receptionTime, postId, encrypted, chatId, authorId);
                        if (MessageDescription.ContainsKey(message.Type) && !message.Contact.IsBlocked && message.Type != MessageType.ContactStatus)
                            contact.SetLastMessagePreview(message);
                        return true;
                    }
#if RELEASE
