...
namespace Telegraph.Droid.Services
{
    public class AndroidNotificationManager : INotificationManager
    {
        ...
        public int ScheduleCallNotification(string title, string message, string chatId, bool isVideoCall, string userName)
        {
            ...
            cancelIntent.PutExtra("videoCallEnable", isVideoCall);
            cancelIntent.PutExtra("chatId", chatId);
            cancelIntent.PutExtra("isCallingByMe", false);
            ...
            var callPendingIntent = PendingIntent.GetBroadcast(AndroidApp.Context, _pendingIntentId, callPageIntent, PendingIntentFlags.CancelCurrent);
            var cancelPendingIntent = PendingIntent.GetBroadcast(AndroidApp.Context, _pendingIntentId, cancelIntent, PendingIntentFlags.OneShot);
        ...
        public int ScheduleOnGoingCallNotification(string title, string chatId, string userName)
        {
            _messageId = 190;
            ...
            cancelIntent.PutExtra("isCallingByMe", false);
            cancelIntent.PutExtra("username", userName);
            cancelIntent.PutExtra("isCallOnGoing", true);
        ...
        private void CreateOnGoingCallNotificationChannel()
        {

            var channelNameJava = new Java.Lang.String(MessageChannelName);
            var channel = new NotificationChannel(MessageChannelId, channelNameJava, NotificationImportance.Low)
            {
                Description = _channelDescription
            };
            channel.SetShowBadge(true);
