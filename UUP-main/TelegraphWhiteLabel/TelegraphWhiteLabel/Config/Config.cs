	...
	internal struct Connection
	{
		...
		private static string GetEntryPoint()
		{
			...
			entryPoint = "http://88.196.134.76/"; //Server a Tallinn
												  //entryPoint = TelegraphChannel.Utility.GetLocalIPAddress(); // Intranet
#else
			try
			{
				var client = new WebClient();
				Stream stream = client.OpenRead("http://88.196.53.119/telegraph/entrypoint.txt");
			...
			if (host == null)
				entryPoint = "http://88.196.134.76/"; //Server a Tallinn
				//entryPoint = "http://88.196.53.119/telegraph/"; // Computer in office
#endif
			return entryPoint;
		}
