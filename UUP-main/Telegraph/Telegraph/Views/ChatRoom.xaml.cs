...
namespace Telegraph.Views
{
    ...
    public partial class ChatRoom : BasePage, IKeyboardHeightChange
    {
            ...
            System.Collections.Specialized.NotifyCollectionChangedEventArgs e)
        {
            ...
            MFCopy.IsVisible = selectedMessages.Count(tuple => tuple.Item1?.Type != MessageFormat.MessageType.Text) == 0;
            ...
            string videoType = null)
        {
            ...
            if (video != null && videoType != null)
                NavigationTappedPage.Context.Messaging.ShareEncryptedContent(currentContact, videoType, video,
        ...
        private void OnMediaFileSelected(List<byte[]> images, ulong? replyToPostId = null)
        {
            ...
            editImagePage.AttachPicture += async (data) =>
            {
                await Task.Run(() =>
                {
                    Parallel.ForEach(data, (image, index) =>
                    {
                        OnSend(replyToPostId: replyToPostId, image: image);
                    });
                });
            };
