...
namespace SecureStorage
{
    ...
    public class ObjectStorage
    {
        ...
        public string[] GetAllKey(Type type)
        {
            ...
            var path = DirectoryName(objFolder);
            try
            {
                var oldPath = path.Replace("EncryptedMessaging", "TelegraphLibrary");
                if (Initializer.IsoStore.DirectoryExists(oldPath))
                    Initializer.IsoStore.MoveDirectory(oldPath, path);
            }
            catch (Exception e)
            {
                var c = e;
            }
            var a = DirectoryName(objFolder);
            if (Initializer.IsoStore.DirectoryExists(path))
