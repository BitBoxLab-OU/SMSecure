        ...
        public string FirebaseToken
        {
            ...
            set
            {
                ...
                if (value != null)
                {
                    ...
                    var lastUpdateTime = Context.SecureStorage.ObjectStorage.LoadObject(typeof(string), "FirebaseTokenUpdateTime") as string;
                    if (tokenChanged || lastUpdateTime == null)
                    {
                        Context.SecureStorage.ObjectStorage.SaveObject(DateTime.Now.ToLocalTime(), "FirebaseTokenUpdateTime");
                        Context.Contacts.ForEachContact(contact =>
                        {
                            Context.Messaging.NotifyFirebaseTokenChange(contact, _firebaseToken);
                        });
                    }
                }
            }
        ...
        public string DeviceToken
        {
            ...
            set
            {
                ...
                if (value != null)
                {
                    ...
                    var lastUpdateTime = Context.SecureStorage.ObjectStorage.LoadObject(typeof(string), "DeviceTokenUpdateTime") as string;
                    if (tokenChanged || lastUpdateTime == null)
                    {
                        Context.SecureStorage.ObjectStorage.SaveObject(DateTime.Now.ToLocalTime().ToString(), "DeviceTokenUpdateTime");
                        Context.Contacts.ForEachContact(contact =>
                        {
                            Context.Messaging.NotifyDeviceTokenChange(contact, _deviceToken);
                        });
                    }
                }
            }
