    ...
    public class Contacts
    {
        ...
        internal void LoadContacts(bool onlyServer = false)
        {
            ...
            if (!Context.My.IsServer && !onlyServer)
            {
                ...
                {
                    Console.WriteLine("Contact name: =--->> " + ((Contact)obj).Name);
        ...
        private bool AddContact(Contact contact, SendMyContact sendMyContact = SendMyContact.Send, bool sendToGroup = true, ContactMessage setSubContact = null)
        {
            ...
            lock (ContactsList)
            {
                ...
                var backUpMyContact = !contact.IsServer && contact.IsVisible;// The first time I add a contact, I make my first cloud backup. It is not backed up first to save space for test accounts
                ContactsList.Add(contact);
        ...
        //#if DEBUG
       // public static string CloudPubKey = @"A4f7EZyD/lVQd5P4r0H3haPCdQJNOU/6sm7LsZoIT+XH";
        //#else
